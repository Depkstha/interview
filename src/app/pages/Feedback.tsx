  ...
  return (
    <MainLayout>
      <Alert variant="destructive" className="mx-auto max-w-md">
        ...
        <AlertTitle>Feedback Feature Currently in Beta</AlertTitle>
      </Alert>
      {interviewSession.interview.aiFeedbackEnabled ? (
        <section className="section-feedback">
          ...
          {callSession?.stereoRecordingUrl && (
            ...
            {feedback?.categoryScores?.map((category, index) => (
              <div key={index}>
                <p className="font-bold">
                  {index + 1}. {category.name} ({Math.round(category.score)}
                  /100)
                </p>
          ...
          <div className="buttons">
            ...
            >
              <p className="text-sm font-semibold text-black text-center flex gap-2 items-center">
        ...
        <section className="section-feedback">
            ...
            <div className="flex flex-col lg:flex-row gap-5">
              ...
              <div className="flex flex-row gap-2 items-center">
                ...
                <p>
                  Overall Impression:{" "}
                  <span className="text-primary-200 font-bold">
                    {feedback?.counselorScore
                      ? Math.round(feedback?.counselorScore)
                      : "-"}
                  </span>
                  /100
                </p>
          ...
          {feedback.counselorScore && feedback.counselorFinalAssessment ? (
            <>
              <p>{feedback?.counselorFinalAssessment}</p>

              {/* Interview Breakdown */}
              <div className="flex flex-col gap-4">
                <h2>Breakdown of the Interview:</h2>
                {feedback?.counselorCategoryScores?.map((category, index) => (
                  <div key={index}>
                    <p className="font-bold">
                      {index + 1}. {category.name} ({Math.round(category.score)}
                      /100)
              ...
              <div className="flex flex-col gap-1 items-end text-right">
                <h4 className="text-sm font-semibold">Counselor</h4>
                <p className="text-base ">– {feedback.counselor.name}</p>
              </div>
            </>
          ) : (
            <Alert variant="destructive" className="mx-auto max-w-full my-5">
              <Terminal className="h-4 w-4" />
              <AlertTitle>Feedback Not Available</AlertTitle>
              <AlertDescription>
                The counselor has not yet provided feedback for this interview.
